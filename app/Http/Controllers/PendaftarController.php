...
class PendaftarController extends Controller
{
    ...
    /**
     * Query dasar + semua filter (search/status/gender/jenjang/lomba) yang dipakai
     * BERSAMA oleh index() (tampilan tabel) dan export() — supaya hasil export selalu
     * konsisten dengan filter yang sedang aktif di halaman, tidak pernah bisa "nyasar"
     * mengekspor semua data padahal tabelnya sudah difilter.
     */
    private function filteredQuery(Request $request)
    {
        ...
        if($request->filled('jenjang')&&in_array($request->jenjang,['SMP','SMA','UMUM'])) {
            $q->whereHas('lomba', fn($lq)=>$lq->where('jenjang',$request->jenjang));
        }
        if($request->filled('id_lomba')) $q->where('id_lomba',$request->id_lomba);
        return $q;
    }
    ...
    {
        $allowed=$this->allowedGenders(); $role=session('admin_user.role');
        $pendaftars=$this->filteredQuery($request)->orderByDesc('created_at')->paginate(20)->withQueryString();
        $lombas=\App\Models\Lomba::whereIn('gender',$allowed)->orderBy('nama_lomba')->get();
        return view('admin.pendaftar.index',compact('pendaftars','allowed','role','lombas'));
    }
    ...
    public function export(Request $request)
    {
        ...
        $rows=$this->filteredQuery($request)->orderBy('gender')->orderBy('created_at')->get();
    ...
    private function toCsv($rows,$suffix,$ts){
        ...
        return response()->stream(function() use($rows){
            ...
            fputcsv($f,['NISN','Nama','Gender','Jenjang','Nama Tim','Sekolah','Lomba','Tipe','HP Peserta','HP Pendamping','Nama Pendamping','Link Twibbon','Status','Tanggal']);
            foreach($rows as $p) fputcsv($f,[$p->nisn,$p->nama,$p->gender,$p->lomba?->jenjang??'-',$p->nama_tim??'-',$p->nama_sekolah,$p->lomba?->nama_lomba??'-',$p->lomba?->tipe??'-',$p->hp_peserta??'-',$p->hp_pendamping??'-',$p->nama_pendamping??'-',$p->link_twibbon??'-',$p->status_verifikasi,$p->created_at->format('Y-m-d H:i')]);
            fclose($f);
    ...
    private function toExcel($rows,$suffix,$ts){
        ...
        $h='<html><head><meta charset="UTF-8"></head><body><table border="1"><tr><th>NISN</th><th>Nama</th><th>Gender</th><th>Jenjang</th><th>Nama Tim</th><th>Sekolah</th><th>Lomba</th><th>Tipe</th><th>HP Peserta</th><th>HP Pendamping</th><th>Pendamping</th><th>Twibbon</th><th>Status</th><th>Tanggal</th></tr>';
        foreach($rows as $p){ $h.='<tr>'; foreach([$p->nisn,$p->nama,$p->gender,$p->lomba?->jenjang??'-',$p->nama_tim??'-',$p->nama_sekolah,$p->lomba?->nama_lomba??'-',$p->lomba?->tipe??'-',$p->hp_peserta??'-',$p->hp_pendamping??'-',$p->nama_pendamping??'-',$p->link_twibbon??'-',$p->status_verifikasi,$p->created_at->format('Y-m-d H:i')] as $v) $h.='<td>'.htmlspecialchars((string)$v).'</td>'; $h.='</tr>'; }
        $h.='</table></body></html>';
    ...
    private function toPdf($rows,$suffix,$ts){
        ...
        $h.='<table><thead><tr><th>#</th><th>NISN</th><th>Nama</th><th>Gender</th><th>Jenjang</th><th>Sekolah</th><th>Lomba</th><th>HP Peserta</th><th>Status</th><th>Tanggal</th></tr></thead><tbody>';
        foreach($rows as $i=>$p){ $badge=match($p->status_verifikasi){'Terverifikasi'=>'<span class="v">Terverifikasi</span>','Ditolak'=>'<span class="t">Ditolak</span>',default=>'<span class="b">Belum</span>'}; $h.='<tr><td>'.($i+1).'</td><td>'.$p->nisn.'</td><td>'.htmlspecialchars($p->nama).'</td><td>'.$p->gender.'</td><td>'.htmlspecialchars($p->lomba?->jenjang??'-').'</td><td>'.htmlspecialchars($p->nama_sekolah).'</td><td>'.htmlspecialchars($p->lomba?->nama_lomba??'-').'</td><td>'.htmlspecialchars($p->hp_peserta??'-').'</td><td>'.$badge.'</td><td>'.$p->created_at->format('d/m/Y').'</td></tr>'; }
